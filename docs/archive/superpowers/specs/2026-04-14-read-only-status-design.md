...
2. 页面只读、不允许输入或多应用列表，内容包括目标包名、目标 DPI、已安装的两个 Hook、静态作用域描述和真机验证提示。
...
- 每个信息块由标题 TextView 和正文 TextView 组成，使用合理的 内边距/spacing，借助 DeviceDefault 主题和 `textAppearance` 样式列出标题与正文，确保内容在竖向滚动时可辨。
- 不提供任何可编辑控件、多包列表或交互菜单，仅用于展示静态信息。

## 页面逻辑
- `MainActivity` 通过 `setContentView(R.layout.activity_status)` 加载 布局，然后依次查找 `R.id.status_target_info`、`R.id.status_hooks` 等正文 TextView。
...
- 验证提示中的消息同样来自字符串资源，提醒团队留意 `DPIS` 记号的 logcat 条目，观察目标应用的 density 是否变为 560，而系统其他应用继续使用默认 DPI。
...
- 页面会强调只有在 `DpiConfig` 指定的目标包才会执行上述 Hook，其他 应用 不会触发 hook 思路，静态 scope 只在目标包内生效。

## 真机验证提示
- 查看 logcat 时，仅保留 `DPIS` TAG，确认出现如下关键词：`installed ResourcesManager and ResourcesImpl hooks`、`override densityDpi in ResourcesManager`、`override densityDpi in ResourcesImpl`。这些 日志 同时确认钩子被触发并修改 `densityDpi`。
- 观察目标包 UI 渲染结果是否等效于 560dpi：可以在目标 应用 的显示设置里看 DPI 指示，或通过截图与默认 DPI 结果对比；也可辅助通过 `DensityOverride` 生成的 `density`/`scaledDensity` 日志。
- 验证其他应用（例如 Settings 以外的系统应用）仍然使用原始 DPI，确保 Hook 仅作用于目标包。

请先审阅这个规范；我写完后会请您在文件里确认，然后继续实施计划。
