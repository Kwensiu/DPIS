...
| --- | --- | --- |
| `diagnostics` | `FeedbackDiagnostic*`, `LogGate`, `DpisLog*`, `LsposedLogReader`, runtime diagnostic parsers | Keep diagnostic package file-role semantics aligned with `CONTEXT.md`. |
...
| `config` | `ConfigSnapshot*`, `PackageConfig*`, `PerAppDisplayConfigSource`, runtime-property config preferences, package hook plan inputs | Owns package-level aggregate config models and read views. `DpisConfigStore` stays root-owned concrete storage. |
...
| `runtime` | `ModuleRuntimeReloadAdvisor`, `ModuleRuntimeStateReporter`, `XposedSelfActivation` | Small runtime lifecycle helpers for module reload advice, system-server load markers, and self-package activation. Public surface is limited to the existing entry methods used by app startup and flavor entry points. |
...
| `fonts` | `ComposeFontRuntimeClassifier`, `FontFieldRewriteMath`, `FontTypefaceLoader`, `PublishedFontFileResolver`, `TextViewFontProvenanceTracker` | Focused font runtime/file helpers. The package move did not change font route ownership or mode semantics. |
...
| `fonts` | `FontLibraryEntry`, `FontLibraryStore`, `FontLibraryActivity`, `FontLibraryConfigStore` | Imported-font library metadata, storage, and UI. `FontLibraryConfigStore` adapts the root-owned `DpisConfigStore` typeface/TTC operations so the Activity no longer needs root package access. |
...
| `about` | `AboutActivity`, `OpenSourceLicenseActivity` | About/update-info and open-source-license components moved together because the about page owns the license entry. Manifest ownership now points at `.about.*` components. |
...
| `quickconfig` | `QuickConfigTileService` | QS tile implementation moved with quick-config ownership. The manifest still exposes root `.QuickConfigTileService` through a compatibility shim so already pinned QS tiles keep their stable `ComponentName`. The Activity stays root-owned until its config-store and diagnostic entry seams are isolated. |
...
| `ui` | `ComposeOverlay` | Activity-owned composition host for remaining Java `show()` dialogs. Visible width and surface come from `ModalDialog`; do not restore `DialogWindowSizer` or `MaterialAlertDialogBuilder`. |
...
| `diagnostics` | `ForegroundAppReader` | Root-backed foreground package snapshot reader for feedback diagnostics. Only `readForegroundPackage()` is public for the coordinator; parser details remain package-private in the diagnostics package. |
| `diagnostics` | `AppLauncher` | Root-backed app restart adapter used by feedback diagnostic flows. Public surface is limited to construction and `restartForDiagnostic`; root command details stay behind `RootAppProcessLauncher`. |
...
| `diagnostics` | `SessionWindow` | Diagnostic log-window value shared by feedback export and LSPosed timeline parsing. Public surface is limited to `around`, timestamp accessors, and `contains`; margin constants remain diagnostics-local. |
| `diagnostics` | `TimelineClassifier` | Runtime/LSPosed diagnostic event classifier. Root callers map feedback requests into a diagnostics `Context`, so the classifier does not depend on root-owned request internals. Event state is exposed through accessors, not public fields. |
| `diagnostics` | `SummaryBuilder` | Feedback summary text formatter. Root coordinator maps its request into a diagnostics `Input`, so summary formatting does not depend on root-owned request or viewport models. |
| `diagnostics` | `LsposedTimelineParser` | LSPosed feedback timeline parser and windowed raw-log formatter. Root export code maps coordinator state into a diagnostics `Input`, so parser ownership moved without exposing app-list, config-store, viewport, or font-mode internals. |
| `diagnostics` | `Coordinator`, `ExportBuilder`, `ResultSheet`, `RuntimeEvents`, `RuntimeHotPathEvents`, `RuntimeSelfTest`, `RuntimeTransport` | Feedback orchestration, package export, result presentation, and runtime capture moved behind the diagnostics package seam. Root UI calls the public coordinator/export/result-sheet interfaces; runtime routes call the public event recorders. Diagnostic internals remain package-local. |
| `diagnostics` | `LogGate` | Diagnostic-log enable gate moved with log/feedback diagnostics. `ConfigStoreFactory.enableDiagnosticLogs` is the root-owned seam for local-store mutation plus `DpisLog` runtime state, so diagnostics does not manipulate logging internals directly. |
| `runtime.delivery` | `RuntimeConfigDelivery`, `RuntimePropertyRecoveryCoordinator`, `RuntimePropertyInstallCleanup`, `RuntimeDebugPropertyBridge`, `RuntimeDebugPropertySyncer` | Runtime property delivery, install cleanup, recovery resync, and debug property publish. Callers keep the existing public entry methods; command construction stays package-private in `runtime.delivery`. |
...
| `quirks` | `WechatDpiPropertyBridge` | WeChat DPI runtime property-name and read bridge moved next to the syncer. Public surface is limited to property-name/read helpers; property lookup and parsing remain private. |
...
| `applist` | `InstalledAppCatalogCoordinator` | Installed-app catalog loading, icon warmup, and app-list row projection moved with app-list ownership. `DpisConfigStore` exposes the read-only package status fields needed by app-list presentation. |
...
| `config` | `ConfigSnapshot`, `ConfigSnapshotLoader`, `ConfigSnapshotStore`, `ConfigPreferenceKeys`, `PackageConfigSnapshot`, `PackageConfigValue`, `PackageConfigRepository`, `PerAppDisplayConfigSource`, `RuntimePropertyConfigPreferences`, `ModulePackagePlan` | Package-level aggregate config model, runtime-property read view, display config source, and hook-plan input model. `DpisConfigStore` implements the narrow snapshot-store seam and keeps concrete persistence/root key mutation in the root package; tests use accessors or config-package white-box tests instead of forcing broader production visibility. |
...
| `runtime.systemserver` | `HyperOsRustProcessHookInstaller` | HyperOS Rust-process environment hook moved with system-server route ownership. Modern/legacy flavor entries still call the public install/legacy adapter methods; route behavior is unchanged. |
| `runtime.font` | `HyperOsFlutterFontBridge` | HyperOS font runtime property-name/read/publish bridge moved with runtime font delivery. Callers use the same public property protocol; `DpisLog` remains the root logging facade. |
| `hyperos` | `HyperOsNativeProxyApplyPolicy`, `HyperOsRustProcessArgPolicy`, `HyperOsNativeRoutePolicy` | JVM-testable HyperOS native-proxy apply/rollback/restart policy, `startRustProcess` argument-slot / environment rewrite policy, and named native-route selection (`PARAGRAPH_BUILDER` / `CONFIGURATION_GOT`). Kept outside `runtime/` so coverage follows the directory contract. |
| `runtime.hyperos` | `HyperOsNativeAppDetector`, `HyperOsNativeProxyFacade`, `HyperOsNativeProxyBindMounter`, `HyperOsNativeProxyAssetExporter`, `HyperOsNativeProxyStatus`, `HyperOsNativeProxyRefreshCoordinator` | HyperOS native-proxy device glue: candidate detection, save/restart facade, bind-mount, bundled so export, status inspect, and the dormant refresh helper. Do not wire automatic refresh into startup or package replacement without revisiting native file side effects. |
| `runtime.presentation` | `ModuleRuntimeReloadNoticeCoordinator` | Activity-bound one-time reload notice. Advisor/state live in `runtime.lifecycle`; the coordinator only owns the host lifecycle. |
| `runtime.transport` | `RootCommandRunner`, `SecureProcessLauncher` | Shared root-command and PATH-pinned process launch used by property syncers, diagnostics, fonts, HyperOS bind-mount, and root probes. |
| `runtime.probe` | `CallerTrace`, `DebugPackageOverride`, `RuntimeClock`, `RuntimeHotPathEvidenceSampler`, `RuntimeDiagnosticLogFingerprint` | Runtime probe helpers: caller formatting, debug package matching, clocks, hot-path sampling, and diagnostic log fingerprint. |
| `runtime.lifecycle` | `ModuleRuntimeReloadAdvisor`, `ModuleRuntimeStateReporter`, `XposedSelfActivation`, `DpisPackageLifecycleReceiver` | Module reload advice, system-server load markers, self-package activation, and the package-lifecycle receiver. Manifest ownership now points at `.runtime.lifecycle.DpisPackageLifecycleReceiver`. `ConfigStoreFactory` and `ProcessScopedInstallGate` stay at the `runtime` root: store construction is shared with UI, and the install gate is shared by app-process, font, and system-server installers. |
| `settings` | `SystemScopeCoordinator`, `SystemHooksToggleController` | System-scope request coordination and system-hook toggle state moved with settings ownership. Public seams are the host/view/gateway protocols used by root UI assembly; service lookup remains behind `DpisApplication`. |
| `appconfig` | `LandAppDetailPaneBinder` | Landscape app-detail editor moved with app-config ownership. MainActivity uses the public binder/actions/state helpers as its app-config panel contract. |
...
| `SystemServerSettingsPageController` | Settings page assembly still owns service listener wiring, backup/import/export, cache cleanup, language, font debug, and multiple row groups. Keep it root-owned until split into smaller settings section controllers. |
