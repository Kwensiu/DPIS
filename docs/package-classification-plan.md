...
| `diagnostics` | `DiagnosticAppLauncher` | Root-backed app restart adapter used by feedback diagnostic flows. Public surface is limited to construction and `restartForDiagnostic`; root command details stay behind `RootAppProcessLauncher`. |
...
| `diagnostics` | `DiagnosticCoordinator`, `DiagnosticExportBuilder`, `DiagnosticResultSheet`, `RuntimeEvents`, `RuntimeHotPathEvents`, `RuntimeSelfTest`, `RuntimeTransport` | Feedback orchestration, package export, result presentation, and runtime capture moved behind the diagnostics package seam. Root UI calls the public coordinator/export/result-sheet interfaces; runtime routes call the public event recorders. Diagnostic internals remain package-local. |
| `diagnostics` | `DiagnosticLogGate` | Diagnostic-log enable gate moved with log/feedback diagnostics. `ConfigStoreFactory.enableDiagnosticLogs` is the root-owned seam for local-store mutation plus `DpisLog` runtime state, so diagnostics does not manipulate logging internals directly. |
