...
| `runtime.lifecycle` | `ModuleRuntimeReloadAdvisor`, `ModuleRuntimeStateReporter`, `XposedSelfActivation`, `ProcessScopedInstallGate`, `DpisPackageLifecycleReceiver` | Module reload advice, system-server load markers, self-package activation, hook-install idempotency, and the package-lifecycle receiver. Manifest ownership now points at `.runtime.lifecycle.DpisPackageLifecycleReceiver`. `ConfigStoreFactory` stays at the `runtime` root as the store-construction seam used by UI, diagnostics, fonts, and this receiver. |
