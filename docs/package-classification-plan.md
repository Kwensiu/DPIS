...
| `MainUiAction`, `MainUiState`, `MainViewModel`, `MainWorkspaceMode` | This is a coherent main UI state machine and should not be moved piecemeal. The old single-case `MainUiEffect` file was merged into `MainViewModel.AppsLoadRequest` because it only represented app-list load requests. |
