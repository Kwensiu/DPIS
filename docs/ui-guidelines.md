...
模板工作区是配置型工作区，不是通用卡片样板。修改 `template_workspace.xml`、`item_quick_template_card.xml` 或 `TemplateSummaryChipBinder` 时，优先保持克制、可扫读、接近原生 Material 的方向；不要把它做成仪表盘、营销卡片或装饰感很重的视觉重设计。

- 全局预填卡片表达“未配置应用的默认填入值”。它不是批量模板操作区，应保留主标题、副标题和右侧编辑入口；编辑入口用圆形图标按钮，并与标题组垂直居中。不要在工作区卡片里重新加入重置或应用动作。
- 快捷模板区域使用设置页分类级别的标题层级；标题和右侧新增入口应保持视觉平衡。新增模板入口可以使用淡底圆形加号按钮，不需要文字按钮。
- 快捷模板卡片比全局预填卡片更紧凑，圆角也应更小。模板卡片不显示更新时间，优先展示名称、配置摘要和必要动作。
- 配置摘要使用低强调 chip，只显示已配置项；未配置维度不占位。缺失字体等异常状态使用警告 chip。没有任何配置时显示轻量空状态。
- 模板卡片操作区左侧放选择应用、编辑等圆形图标按钮，右侧保留主要文字操作。纯图标按钮优先使用 `AppCompatImageButton`、`centerInside` 和明确的 `contentDescription`，避免 icon-only `MaterialButton` 出现图标视觉偏移。
- 模板工作区内不要显示 app 列表工作区专属的搜索或帮助悬浮按钮。
