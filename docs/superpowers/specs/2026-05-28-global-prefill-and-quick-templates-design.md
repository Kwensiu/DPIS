...
- `BatchScopeRequestCoordinator`: sends at most one modern batch scope
...
8. For modern, if scope state is known and there are successfully saved
   packages not in scope, call `requestScope(missingPackages)` once.
9. For legacy, unavailable service, unknown scope state, or request failure,
...
- `E:\System\Documents\GitHub\DPIS\app\src\modern\java\com\dpis\module\ModuleMain.java`

Flavor-specific compat API entry:

- `E:\System\Documents\GitHub\DPIS\app\src\legacy\java\com\dpis\module`
...
| `[✓]` | Backup and import integration | Include global prefill and quick template keys in config backup/import while still excluding font binary files and runtime/debug state. | `app/src/main/java/com/dpis/module/DpiConfigStore.java`, `ConfigBackupCodec.java`, `SystemServerSettingsActivity.java`; tests in `DpiConfigStoreTest` or new backup tests | Verified with `./gradlew :app:testModernDebugUnitTest --tests com.dpis.module.DpiConfigStoreTest`: backup tests prove prefill/templates are exported/imported and font binaries are not included. |
| `[✓]` | Global prefill sheet state | For completely unconfigured apps, construct temporary app sheet state from global prefill without persisting or publishing until save. Add generic preview/unsaved state UI in the sheet. | `MainActivity.java`, `InstalledAppCatalogCoordinator.java`, `AppListItem.java`, `AppConfigDialogBinder.java`, `AppConfigSaveHandler.java`, `dialog_app_config.xml`, `strings.xml` | Verified with `./gradlew :app:testModernDebugUnitTest --tests com.dpis.module.AppConfigPrefillPreviewTest --tests com.dpis.module.AppConfigSaveHandlerTest` and `./gradlew :app:testModernDebugUnitTest --tests com.dpis.module.MainActivitySourceSmokeTest --tests com.dpis.module.AppConfigDialogBinderSourceSmokeTest`: tests prove configured apps ignore prefill, unconfigured apps display prefill only as preview, saving converts preview into real config, and the preview UI is wired. |
| `[✓]` | Workspace navigation UI | Add bottom `应用 / 模板` workspace switch, preserve current app workspace behavior, and hide or replace app list tabs while template workspace is active. | `activity_status.xml`, `MainActivity.java`, `MainUiState.java`, possible new `MainWorkspaceMode.java`, `strings.xml`, `dimens.xml`, layout smoke tests | Verified with `./gradlew :app:testModernDebugUnitTest --tests com.dpis.module.MainActivityLayoutSmokeTest --tests com.dpis.module.MainActivitySourceSmokeTest --tests com.dpis.module.MainViewModelTest --tests com.dpis.module.StringResourceParityTest` and `./gradlew :app:testAllDebugUnitTests`: source/layout/state tests prove both workspace entries exist and app tabs/pager/filter controls are hidden in template workspace. |
| `[✓]` | Template workspace cards | Render global prefill fixed card and quick template cards with summaries, missing-font state, and actions `应用 / 编辑 / 选择`. | `TemplateWorkspaceBinder.java`, `QuickTemplateListAdapter.java`, `TemplateConfigSummaryFormatter.java`; resources under `app/src/main/res/layout/` and `values/` | Verified with `./gradlew :app:testModernDebugUnitTest --tests com.dpis.module.TemplateConfigSummaryFormatterTest --tests com.dpis.module.TemplateWorkspaceLayoutSmokeTest --tests com.dpis.module.MainActivityLayoutSmokeTest --tests com.dpis.module.MainActivitySourceSmokeTest --tests com.dpis.module.StringResourceParityTest`: formatter behavior and UI/source/layout smoke tests prove card structure, actions, strings, store reads, and missing-font display hooks exist. |
| `[✓]` | Global prefill edit page | Add dedicated `全局预填` edit page using app-config validation semantics, without scope/process/enable controls. | Candidate `GlobalPrefillActivity.java` or page controller, shared form binder, new layout under `app/src/main/res/layout/` | Verified with `./gradlew :app:testModernDebugUnitTest --tests com.dpis.module.GlobalPrefillSaveHandlerTest --tests com.dpis.module.GlobalPrefillActivitySourceSmokeTest --tests com.dpis.module.MainActivitySourceSmokeTest --tests com.dpis.module.TemplateWorkspaceLayoutSmokeTest --tests com.dpis.module.StringResourceParityTest` and `./gradlew :app:testAllDebugUnitTests`: tests prove save/reset stay on `default_config.*`, existing package config and `target_packages` remain untouched, and the dedicated page exists without scope/process/enable controls. |
| `[✓]` | Quick template edit page | Add dedicated template edit page for name and reusable config values. Preserve selected packages when config values change. | `app/src/main/java/com/dpis/module/QuickTemplateEditActivity.java`, `QuickTemplateSaveHandler.java`, `app/src/main/res/layout/activity_quick_template_edit.xml`, quick-template strings/resources | Verified with static sub-agent review after fixes and targeted tests. Tests prove name validation, config validation, save/update behavior, selected package preservation, explicit viewport apply mode exposure, stale edit ID handling, and absence of scope/process/enable controls. |
| `[✓]` | Quick template app selection page | Add per-template app selection page with installed app list, selection persistence, search/filter if reused, and `已配置` badge for configured apps. | `app/src/main/java/com/dpis/module/QuickTemplateTargetSelectionActivity.java`, `QuickTemplateTargetAdapter.java`, `app/src/main/res/layout/activity_quick_template_targets.xml`, `item_quick_template_target_app.xml` | Verified with static sub-agent review after fixes and targeted tests. Tests prove selected packages are persisted through `QuickTemplateStore.setSelectedPackages`, stale selections are pruned against installed apps, configured apps are identified via `DpiConfigStore.hasRealPackageConfig`, and no template ownership semantics are introduced. |
| `[✓]` | Batch apply coordinator | Apply selected template values to selected packages, overwrite configured apps after confirmation, publish runtime properties for successful writes, and summarize partial failures. | `app/src/main/java/com/dpis/module/QuickTemplateApplyCoordinator.java`, `MainActivity.java`, `TemplateWorkspaceBinder.java`; runtime sync through `ViewportPropertySyncer`, `FontRuntimePropertySyncer`, `FontHookDomainPropertySyncer` | Verified with static sub-agent review after fixes and `./gradlew :app:testModernDebugUnitTest --tests com.dpis.module.QuickTemplateApplyCoordinatorTest --tests com.dpis.module.QuickTemplateApplySourceSmokeTest --tests com.dpis.module.StringResourceParityTest`. Tests prove multi-package writes, overwrite counting, partial failure summary, empty selection blocking, installed-package filtering at apply time, full runtime publishing/clearing for successful writes, no `失败 0 个` copy, and no process/HyperOS proxy side effects. |
| `[✓]` | Batch scope request | For modern, send at most one `requestScope(List)` after successful batch writes. For legacy/service unavailable/unknown scope, keep config and show manual-scope guidance. | `app/src/main/java/com/dpis/module/BatchScopeRequestCoordinator.java`, `MainActivity.java`; existing single-app `SystemScopeCoordinator.java` remains unchanged | Verified with static sub-agent review and `./gradlew :app:testModernDebugUnitTest --tests com.dpis.module.BatchScopeRequestCoordinatorTest --tests com.dpis.module.BatchScopeRequestCoordinatorSourceSmokeTest --tests com.dpis.module.StringResourceParityTest`. Tests/review prove one batch request call, no singleton loop fallback, request only after successful writes, partial approved callback refreshes actual scope state, and compat/service/unknown-scope paths keep saved config with manual-scope guidance. |
| `[✓]` | Strings, dimensions, and UI polish | Add localized English and Simplified Chinese strings, semantic dimensions, accessible labels, and confirmation copy. Follow `docs/ui-guidelines.md`. | `app/src/main/res/values/strings.xml`, `values-zh-rCN/strings.xml`, `dimens.xml`, quick-template layouts, `bg_quick_template_configured_badge.xml` | Verified with static sub-agent review after fixes and `./gradlew :app:testModernDebugUnitTest --tests com.dpis.module.QuickTemplateUiPolishSmokeTest --tests com.dpis.module.StringResourceParityTest --tests com.dpis.module.TemplateWorkspaceLayoutSmokeTest --tests com.dpis.module.QuickTemplateTargetSelectionSourceSmokeTest --tests com.dpis.module.QuickTemplateEditActivitySourceSmokeTest --tests com.dpis.module.GlobalPrefillActivitySourceSmokeTest --tests com.dpis.module.AppConfigDialogBinderSourceSmokeTest`. Tests/review prove localized copy parity, `快捷模板` terminology, semantic quick-template dimensions, accessible mode toggles, stacked toggle thumb layout, confirmation copy, and no hard-coded user-visible text in new quick-template XML where resources are expected. |
| `[x]` | End-to-end verification | Run focused tests, full debug unit tests, then perform real-device or screenshot validation for main workspace, edit pages, selection page, apply confirmation, and preview sheet state. | Test suite and runtime validation evidence; no production code required unless bugs are found | Focused quick-template tests and `./gradlew :app:testAllDebugUnitTests` pass as of this implementation pass. Real-device/screenshot validation is still pending review/acceptance before `[✓]`. |
...
- modern sends one `requestScope(List)` call;
- legacy or service unavailable shows manual-scope guidance;
...
- Applying a quick template overwrites selected configured apps only after
  second confirmation.
- modern batch apply sends at most one scope request for missing-scope
  packages.
- legacy and unavailable-service paths keep saved config and guide manual
  scope selection.
