...
| `[x]` | Strings, dimensions, and UI polish | Add localized English and Simplified Chinese strings, semantic dimensions, accessible labels, and confirmation copy. Follow `docs/ui-guidelines.md`. | `app/src/main/res/values/strings.xml`, `values-zh-rCN/strings.xml`, `dimens.xml`, quick-template layouts, `bg_quick_template_configured_badge.xml` | Implemented with resource/source smoke coverage; pending review/acceptance before `[✓]`. `StringResourceParityTest` and quick-template polish smoke checks cover localized copy, semantic dimensions, accessible back labels, confirmation copy, and no hard-coded user-visible text in new quick-template XML where resources are expected. |
