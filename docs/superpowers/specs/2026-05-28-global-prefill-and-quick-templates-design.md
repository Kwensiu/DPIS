...
| `[x]` | Quick template app selection page | Add per-template app selection page with installed app list, selection persistence, search/filter if reused, and `已配置` badge for configured apps. | `app/src/main/java/com/dpis/module/QuickTemplateTargetSelectionActivity.java`, `QuickTemplateTargetAdapter.java`, `app/src/main/res/layout/activity_quick_template_targets.xml`, `item_quick_template_target_app.xml` | Implemented with source/layout smoke coverage; pending review/acceptance before `[✓]`. Tests prove selected packages are persisted through `QuickTemplateStore.setSelectedPackages`, configured apps are identified via `DpiConfigStore.hasRealPackageConfig`, and no template ownership semantics are introduced. |
| `[x]` | Batch apply coordinator | Apply selected template values to selected packages, overwrite configured apps after confirmation, publish runtime properties for successful writes, and summarize partial failures. | `app/src/main/java/com/dpis/module/QuickTemplateApplyCoordinator.java`, `MainActivity.java`, `TemplateWorkspaceBinder.java`; runtime sync through `FontRuntimePropertySyncer` | Implemented with focused unit/source smoke coverage; pending review/acceptance before `[✓]`. Unit tests prove multi-package writes, overwrite counting, partial failure summary, empty selection blocking, successful-write runtime publishing, no `失败 0 个` copy, and no process/HyperOS proxy side effects. |
