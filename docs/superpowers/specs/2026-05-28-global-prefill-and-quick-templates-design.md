...
| `[x]` | Batch scope request | For modern101, send at most one `requestScope(List)` after successful batch writes. For compat100/service unavailable/unknown scope, keep config and show manual-scope guidance. | `app/src/main/java/com/dpis/module/BatchScopeRequestCoordinator.java`, `MainActivity.java`; existing single-app `SystemScopeCoordinator.java` remains unchanged | Implemented with unit/source smoke coverage; pending review/acceptance before `[✓]`. Tests prove one batch request call, no singleton loop fallback, partial approved callback triggers refresh, and compat/service/unknown-scope paths keep saved config with manual-scope guidance. |
