...
- Some project-owned fixed labels, such as the Russian language name, are filled
  by the script and are not listed as missing.
...
- 少量项目维护的固定标签，例如俄语语言名，会由脚本补入，不会出现在缺失列表里。
