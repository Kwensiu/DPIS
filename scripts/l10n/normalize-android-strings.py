...
def render_string_line(entry: StringEntry) -> str:
    ...
    translatable = "" if entry.translatable else ' translatable="false"'
    return f'    <string name="{entry.name}"{translatable}>{value}</string>'
...
def main() -> int:
    ...
    translatable_base_names = [entry.name for entry in base_entries if entry.translatable]
    non_translatable_names = {entry.name for entry in base_entries if not entry.translatable}
    ...
    pruned_non_translatable = [
        entry.name for entry in source_entries if entry.name in non_translatable_names
    ]
    missing = [
        name for name in translatable_base_names
        if name not in source_name_set and name not in maintained_name_set
    ]
    empty_values = [
        entry.name for entry in source_entries
        if entry.name in base_name_set
        and entry.name not in non_translatable_names
        and entry.value == ""
    ]

    placeholder_mismatches: list[str] = []
    for name in translatable_base_names:
    ...
    if args.missing_report:
        write_missing_report(
            args.missing_report,
            (entry for entry in base_entries if entry.translatable),
            source_by_name,
            maintained_values,
        )
    ...
    print_list("Base non-translatable keys", sorted(non_translatable_names))
    print_list("Pruned non-translatable keys", sorted(set(pruned_non_translatable)))
    print_list("Maintainer-provided keys", [name for name in translatable_base_names if name in maintained_values])
