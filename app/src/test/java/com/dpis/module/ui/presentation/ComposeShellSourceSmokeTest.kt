...
class ComposeShellSourceSmokeTest {
    ...
    fun composeThemeAndShellKeepTheRequiredBoundaries() {
        ...
        assertTrue(feedback.contains("fun outlinedWarningButtonColors("))
        assertTrue(feedback.contains("fun dangerButtonColors("))
        ...
        assertTrue(localized.contains("applyComposeWindowBackground()"))
        ...
        assertTrue(mainShell.contains("BackHandler(enabled = homeDestination != null && selectedDestination != homeDestination)"))
        assertTrue(mainShell.contains("fun workspaceHomeDestination("))
        val startup = read(
        ...
        assertTrue(mainShellHost.contains("dynamicColor = appearance.dynamicColorEnabled"))
        assertTrue(mainShellHost.contains("themeColor = appearance.themeColor"))
        assertTrue(mainShellHost.contains("paletteStyle = appearance.paletteStyle"))
        assertTrue(mainShellHost.contains("colorSpecification = appearance.colorSpecification"))
    }
    ...
    fun interfaceScaleSliderShowsOnlyEndpointMarkers() {
        ...
        val themeRows = read("src/main/java/com/dpis/module/settings/presentation/ThemeRows.kt")
        ...
        assertTrue(themeRows.contains("steps = 0"))
        assertTrue(themeRows.contains("SliderDefaults.Track"))
        assertTrue(themeRows.contains("latestGestureValue.floatValue"))
        assertTrue(themeRows.contains("AppUiScaleManager.normalizeSliderPercent(changedValue)"))
        assertTrue(themeRows.contains("HapticFeedbackConstants.CLOCK_TICK"))
        assertTrue(theme.contains("style = MaterialTheme.typography.titleMedium"))
        assertFalse(settings.contains("SettingsScaleRow("))
        assertTrue(settings.contains("SettingsChoiceMenu("))
        assertTrue(settings.contains("SettingsChoiceRow("))
        assertTrue(settings.contains("selectedLanguageLabel"))
        assertFalse(settings.contains("LanguageDialogContent("))
        ...
        assertTrue(home.contains("titleRes = R.string.app_name"))
        assertFalse(home.contains("subtitle = {"))
        assertFalse(home.contains("home_workspace_subtitle"))
    ...
    fun standaloneSettingsPagesUseSharedSecondaryPageChrome() {
        ...
        assertTrue(scaffold.contains("initialHeightOffsetLimit = if (collapsedInitially) collapsedOffsetPx else -Float.MAX_VALUE"))
        assertTrue(scaffold.contains("initialHeightOffset = collapsedOffsetPx"))
        assertTrue(scaffold.contains("LargeFlexibleAppBarWithoutSubtitleExpandedHeight"))
        assertFalse(scaffold.contains("deferBodyUntilFirstFrame"))
        assertFalse(scaffold.contains("withFrameNanos"))
        assertFalse(scaffold.contains("topAppBarState.heightOffset = topAppBarState.heightOffsetLimit"))
    ...
    fun themeSettingsExpandsStaticColorOptionsWhenDynamicColorIsDisabled() {
        val theme = read("src/main/java/com/dpis/module/settings/presentation/ThemeSettingsContent.kt")
        val themeRows = read("src/main/java/com/dpis/module/settings/presentation/ThemeRows.kt")
        val host = read("src/main/java/com/dpis/module/settings/presentation/ThemeHost.kt")
        val settings = read("src/main/java/com/dpis/module/settings/presentation/SettingsWorkspaceContent.kt")
        val choiceMenu = read("src/main/java/com/dpis/module/settings/presentation/SettingsChoiceMenu.kt")
        ...
        assertTrue(themeRows.contains("R.string.settings_theme_color_label"))
        ...
        assertTrue(themeRows.contains("ThemeColorOption(ThemeModeStore.DEFAULT_STATIC_THEME_COLOR)"))
        assertTrue(themeRows.contains("ThemeSwatchPreviewCache.prefetch("))
        assertTrue(themeRows.contains("ThemeSwatchPreviewCache.peek("))
        assertFalse(themeRows.contains("ThemeSwatchPreviewCache.get("))
        assertTrue(settings.contains("ThemeSwatchPreviewCache.prefetch("))
        assertTrue(theme.contains("ThemeChoiceMenuAnchor("))
        assertTrue(themeRows.contains("SettingsChoiceMenu("))
        assertTrue(choiceMenu.contains("DropdownMenuPopup("))
        assertTrue(choiceMenu.contains("SelectableDropdownMenuItem("))
        assertTrue(choiceMenu.contains("MenuDefaults.itemShape(index, options.size)"))
        assertTrue(choiceMenu.contains("containerColor = Color.Transparent"))
        assertTrue(choiceMenu.contains("disabledContainerColor = Color.Transparent"))
        assertFalse(choiceMenu.contains("selectedContainerColor"))
        assertTrue(choiceMenu.contains(".widthIn(max = 360.dp)"))
        assertTrue(choiceMenu.contains("shape = MaterialTheme.shapes.large"))
        assertFalse(choiceMenu.contains(".clip(MaterialTheme.shapes.large)"))
        assertFalse(choiceMenu.contains("import androidx.compose.material3.DropdownMenuItem"))
        assertTrue(choiceMenu.contains("rememberClickValueAction(onSelected)"))
        assertTrue(choiceMenu.contains("Box(Modifier.align(Alignment.BottomEnd))"))
        assertFalse(choiceMenu.contains("trailingIcon = if (selected == value)"))
        assertFalse(theme.contains("ThemeChoiceDialog("))
        assertFalse(theme.contains("ThemeModeDialog("))
        assertFalse(themeRows.contains("ThemeColorOption(\"default\")"))
        assertTrue(themeRows.contains("internal fun ThemeSegmentedSurfaceRow("))
        ...
        assertTrue(themeRows.contains("R.string.settings_page_predictive_back"))
        ...
        assertTrue(host.contains("PageSettingsStore.setPredictiveBackEnabled("))
        assertTrue(themeRows.contains("SegmentedListItem("))
        assertTrue(themeRows.contains("shapes = dpisSegmentedShapes(index, total)"))
        assertTrue(themeRows.contains("rememberSegmentedPressedShape("))
        assertTrue(host.contains("ThemeModeStore.setDynamicColorEnabled(context, enabled)"))
        assertTrue(host.contains("dynamicColorEnabled = enabled"))
        assertTrue(host.contains("var mode by remember"))
        assertTrue(host.contains("fun ThemeSettingsRoute("))
        assertTrue(host.contains("ThemeModeStore.resolveDarkTheme(mode, isSystemInDarkTheme())"))
        assertTrue(host.contains("markAppearanceAppliedInPlace()"))
        assertFalse(
            host.substringAfter("val applyMode")
                .substringBefore("val applyDynamicColor")
                .contains("recreate()"),
        )
    ...
    fun composeAppSheetPreservesPartialExpandAndLegacyChromeSemantics() {
        ...
        assertTrue(coordinator.contains("AppConfigWizardHint("))
        val wizardHint = read(
                "src/main/java/com/dpis/module/appconfig/presentation/AppConfigWizardHint.kt")
        assertTrue(wizardHint.contains("R.string.dialog_advanced_wizard_hint"))
        assertTrue(wizardHint.contains("Canvas("))
        assertTrue(wizardHint.contains("wizardHintSurface()"))
        assertTrue(wizardHint.contains("color = hintSurface"))
        assertFalse(wizardHint.contains("bg_app_config_wizard_arrow"))
        ...
        assertTrue(chip.contains("MaterialTheme.colorScheme.secondaryContainer"))
        assertTrue(chip.contains("MaterialTheme.colorScheme.onSecondaryContainer"))
        assertTrue(chip.contains("MaterialTheme.colorScheme.secondary"))
        assertTrue(chip.contains("dangerButtonColors()"))
        assertTrue(chip.contains("warningButtonColors()"))
        assertTrue(chip.contains("successButtonColors()"))
        assertFalse(chip.contains("R.color.dpis_"))
    ...
    fun hookChainIsARecoverableChildPageOfBothEditorSessions() {
        ...
        assertTrue(page.contains("HookChainPageTokens.RiskLow"))
        assertTrue(page.contains("hookDomainStatusColor("))
        assertFalse(page.contains("R.color.font_hook_domain_risk"))
        ...
        assertTrue(page.contains(".clip(shapes.shape)"))
