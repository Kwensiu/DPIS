...
final class FeedbackDiagnosticLsposedTimelineParser {
    ...
    static List<String> parse(
        ...
        FeedbackDiagnosticTimelineClassifier.Context context =
                FeedbackDiagnosticTimelineClassifier.Context.from(request);
        for (DpisLogEntry entry : DpisLogParser.parseLsposedDpis(raw)) {
            ...
            if (!matchesTarget(entry, request)) {
                continue;
            }
            String hotPathEvent = formatHotPathEvent(timestampMillis, entry, request);
            ...
            if (event != null) {
                events.add(formatEvent(timestampMillis, entry, request, event));
    ...
    static void sortTimelineEvents(List<String> events) {
    ...
    private static String formatHotPathEvent(
        ...
        String packageName = fieldValue(
                hotPathMessage,
                "package",
                request != null ? request.packageName : "unknown"
        );
    ...
    static WindowedRawLog windowRawLog(
    ...
    private static boolean matchesTarget(
        ...
        String packageName = request != null ? request.packageName : "";
    ...
    private static String formatEvent(
        ...
        return formatTime(timestampMillis)
                ...
                + " stage=" + event.stage
                + " level=" + event.level
                + " package=" + valueOrDefault(request != null ? request.packageName : "", "unknown")
                + " process=" + valueOrDefault(entry.process, "unknown")
    ...
    static long resolveTimestampMillis(String timestamp, long anchorMillis) {
    ...
    static final class WindowedRawLog {
        final String output;
        final int totalParsed;
        final int droppedOutsideWindow;
        final int droppedUnparsed;
        final int droppedNonDpis;
